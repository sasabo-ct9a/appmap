...
const JA: Copy = {
  ...
  noTestFrameworkHint:
    "自動テストとは、プログラムが「アプリが壊れていないか」を毎回自動で確かめてくれる仕組みです。今は手で触って確認できていれば大丈夫ですが、要素や機能が増えると手動だけでは追いつかなくなります。まずは「主要な要素がちゃんと開くか」を確かめる 1 件だけでも用意しておくと安心です。",
  ...
  noTestsFix: (stack) => {
    ...
    return [
      "まずは Happy Path(正常系)を 1 つ:入口の要素から主要機能まで動くか検証するテスト",
      "AI コーディングツールに「◯◯要素の happy path テストを書いて」と依頼",
