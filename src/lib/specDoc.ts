 ...
 *   ◐ AI解析 … AI がコードを読み取った内容(画面・遷移・操作・データ)。実物と要照合。
...
export function buildSpecDoc(opts: BuildOptions): string {
  ...
  if (eng) {
    ...
    L.push(
      "> このアプリのコードから自動で作った「実際の中身」の説明です。画面やデータは AI が読み取ったものなので、念のため実物と見比べてください。",
    );
  }
  L.push("");
  L.push("---");
  ...
  // ========== 2. 機能要件(画面と操作)(◐) ==========
  // 画面設計と機能要件は同じ「画面」情報なので 1 表にまとめる。
  H("機能要件(画面と操作)", MARK.ai);
  L.push("");
  L.push(
    eng
      ? "AI がコードから読み取った画面と、各画面でできることの一覧です(**実物と要照合**)。"
      : "アプリにある画面と、それぞれで何ができるかの一覧です。",
  );
  L.push("");
  L.push("| 画面 | 何をする画面か | 主な操作 | 遷移先 |");
  L.push("|---|---|---|---|");
  for (const n of sortedNodes) {
    ...
    const outs = (outgoing.get(n.id) ?? [])
      ...
      .join(" / ");
    L.push(
      `| ${escTable(name)} | ${desc || "-"} | ${escTable(acts) || "(表示中心の画面)"} | ${escTable(outs) || "-"} |`,
    );
  }
  L.push("");
  L.push("---");
  ...
  if (screens.edges.length > 0) {
    ...
    L.push(
      "画面のつながりは検出されませんでした(単一画面のアプリか、遷移が読み取れなかった可能性)。",
    );
  }
  L.push("");
  L.push("---");
  ...
  if (infoTerms.size > 0) {
    ...
    L.push(
      "扱う情報は特定できませんでした。各画面で入力・表示・保存する情報をここに追記してください。",
    );
  }
  L.push("");
  if (eng) {
  ...
  L.push("");
  L.push("- 画面名・機能名が、実際のアプリでの呼び方と合っているか");
  L.push("- 画面のつながり(遷移)が、実際の操作順と合っているか");
